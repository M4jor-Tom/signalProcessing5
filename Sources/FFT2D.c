...
Complexe **FFT2D(Complexe **xN, int tailleL, int tailleH, int inverse)
{
	...
	Complexe **XN = malloc(sizeof(Complexe *) * tailleH);
	
	int i;
	for(i = 0; i < tailleH; i++)
	{
		XN[i] = malloc(sizeof(Complexe) * tailleL);
...
{
	//Allocation lignes
	Complexe **ret = malloc(sizeof(Complexe * ) * taille1);
	
	int i, j;
	for(i = 0; i < taille1; i++)
		//Allocation cases
		ret = malloc(sizeof(Complexe) * taille2);
