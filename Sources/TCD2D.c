...
float **TCD2D(float **xN, int tailleL, int tailleH, int inverse)
{
	...
	float **XN = (float **)safeMalloc(sizeof(float *) * tailleH);
	
	int i;
	for(i = 0; i < tailleH; i++)
	{
		XN[i] = (float *)safeMalloc(sizeof(float) * tailleL);
...
{
	//Allocation lignes
	float **ret = (float **)safeMalloc(sizeof(float * ) * taille1);
	
	int i, j;
	for(i = 0; i < taille1; i++)
		//Allocation cases
		ret = (float *)safeMalloc(sizeof(float) * taille2);
